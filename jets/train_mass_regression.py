...
from jets_dataset import JetsMassRegressionDataset
...
def init(args):
    if not exists(args.dir_path + '/mass_regression/'):
        mkdir(args.dir_path + '/mass_regression/')

    args_dict = vars(args)
    dirs = ['models', 'losses', 'args', 'outs', 'Gs']
    for dir in dirs:
        args_dict[dir + '_path'] = args.dir_path + '/mass_regression/' + dir + '/'
    ...
    try: mkdir(args.Gs_path + args.G)
    except FileExistsError: logging.debug("Gs dir exists")
...
def main(args):
    ...
    train_dataset = JetsMassRegressionDataset(args, train=True)
    test_dataset = JetsMassRegressionDataset(args, train=False)
    ...
    def plot_losses(epoch, train_losses, test_losses):
        ...
        try: remove(args.losses_path + args.name + "/" + str(epoch - 1) + ".pdf")
    ...
    def save_model(epoch):
        torch.save(C.state_dict(), args.models_path + args.name + "/C_" + str(epoch) + ".pt")
    ...
    def test(epoch):
        ...
        logging.info(args.outs_path + args.name + '.txt')
