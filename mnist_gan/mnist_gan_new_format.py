...
for k in range(X_train_pre.shape[0]):
    sample = []
    for i in range(X_train_pre.shape[1]):
        for j in range(X_train_pre.shape[2]):
            sample.append([i, j, X_train_pre[k, i, j]])
...
def save_models(generator, discriminator, name, epoch):
    generator.save('models2/%sgan_generator_epoch_%d.h5' % (name, epoch))
    discriminator.save('models2/%sgan_discriminator_epoch_%d.h5' % (name, epoch))
...
def disp_sample_outputs(generator, num_cols, num_rows, name, epoch):
    ...
    for i in range(1, num_cols*num_rows+1):
        ...
        plt.imshow(gen_out[i-1].T[2].reshape(28, 28), cmap=cm.gray_r, interpolation='nearest')
        plt.axis('off')
    plt.savefig("figs2/"+name + "_" + str(epoch) + ".png")
    plt.show()
...
def run_trial(name, epochs):
    ...
    plt.plot(disc_loss)
    # plt.ylim((0,10))
    plt.savefig("figs2/" + name + "_disc_loss.png")
    plt.show()

    plt.figure()
    plt.plot(gan_loss)
    # plt.ylim((0,10))
    plt.savefig("figs2/" + name + "_gan_loss.png")
    plt.show()
